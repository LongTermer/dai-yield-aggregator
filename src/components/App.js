...
class App extends Component {
  ...
  async loadBlockchainData(dispatch) {
    ...
    const aggregator = new web3.eth.Contract(
      Aggregator.abi,
      Aggregator.networks[networkId].address,
    )

    if (!aggregator) {
      window.alert(
        'Aggregator smart contract not detected on the current network. Please select another network with Metamask.',
      )
      return
    }
    ...
    const cDAI_contract = new web3.eth.Contract(
      cDAI_ABI,
      this.state.cDAI_address,
    )

    this.setState({ cDAI_contract })

    const aaveLendingPool_contract = new web3.eth.Contract(
      AAVE_ABI,
      this.state.aaveLendingPool_address,
    )
  ...
  async loadAccountInfo() {
    let walletBalance = await this.state.dai.methods
      .balanceOf(this.state.account)
      .call()
    let aggregatorBalance = await this.state.aggregator.methods
      .amountDeposited()
      .call()

    walletBalance = this.state.web3.utils.fromWei(walletBalance, 'ether')
    aggregatorBalance = this.state.web3.utils.fromWei(
      aggregatorBalance,
      'ether',
    )
    ...
    if (aggregatorBalance !== '0') {
      let activeProtocol = await this.state.aggregator.methods
        .balanceWhere()
        .call()
      activeProtocol === this.state.cDAI_address
  ...
  async depositHandler() {
    ...
    const amount = this.state.web3.utils.toWei(
      this.state.amountToDeposit.toString(),
      'ether',
    )
    ...
    this.state.dai.methods
      .approve(this.state.aggregator._address, amount)
      .send({ from: this.state.account })
      .on('transactionHash', () => {
        this.state.aggregator.methods
          ...
          <main role="main" className="col-lg-12 text-center">
            ...
            <div className="row content">
              <div className="col user-controls">
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      this.setState({ amountToDeposit: e.target.value })
                    }
                  />
                  <button
                    type="submit"
              ...
              <div className="col user-stats">
                <p>
                  <strong>Current Wallet Balance (DAI):</strong>{' '}
                  {this.state.walletBalance}
                </p>
                <p>
                  <strong>Amount Deposited to Aggregator (DAI):</strong>{' '}
                  {this.state.aggregatorBalance}
                </p>
                <p>
                  <strong>Active Protocol:</strong> {this.state.activeProtocol}
                </p>
              </div>
            </div>
          </main>
